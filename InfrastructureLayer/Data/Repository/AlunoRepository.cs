	...
	public class AlunoRepository : IAlunoRepository
	{
		...
		public async Task Registra(AlunoCadastroViewModel viewModel)
		{
			...
			var parameters = new
			{
				viewModel.Nome,
				viewModel.Matricula,
				DataNascimento = viewModel.DataNascimento.ToString("yyy-MM-dd")
			};
		...
		public async Task Atualiza(Aluno aluno)
		{
			...
			var parameters = new
			{
				aluno.Nome,
				aluno.Matricula,
				DataNascimento = aluno.DataNascimento.ToString("yyy-MM-dd")
			};
		...
		public async Task RegistraNotas(AlunoNotasViewModel viewModel)
		{
			...
			var query = new StringBuilder("INSERT INTO NotaAluno(AlunId, Nota) VALUES ");
		...
		{
			_logger.LogInformation($"[AlunoRepository]-[BuscaNotas] -> [Start] : Nome -> {matricula}");

			using var connection = _context.CreateConnection;

			const string query = "SELECT * FROM AlunoNota WHERE AlunoId = (SELECT Id FROM aluno WHERE Matricula = @Matricula)";
		...
		{
			_logger.LogInformation($"[AlunoRepository]-[BuscaAlunoNotas] -> [Start] : Nome -> {matricula}");
